...
class TurtleBot3Env(Node, gym.Env):  # Class that inherits from Node and gym.Env
    ...
    def step(self, action):
        ...
        cmd_vel.linear.x = float(action[0][0][0])
        cmd_vel.angular.z = float(action[0][0][1])
        ...
        # Reward function
        cone_location = np.array([1.721, -0.469])  # Change according to where the cone is
        ...
        r_goal = 1 / (goal_dist + 1e-3)  # Reward the robot for being closer to the goal (avoid division by zero)
